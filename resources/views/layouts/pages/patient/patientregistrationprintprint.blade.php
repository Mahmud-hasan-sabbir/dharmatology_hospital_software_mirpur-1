   ...
   <main style="margin-top: 25px">
      ...
      <div class="row">
        <div class="col-md-4" style="margin-left: 20px">
            <div class="row">
            <p class="op">Patient ID: {{ $id }}</p>
            <p class="op">Patient Name: {{ $patientprint->name ?? 'N/A' }}</p>
            <p class="op">Phone no: {{ $patientprint->telephone ?? 'N/A' }}</p>
            <p class="op">Age: {{ $patientprint->age ?? 'N/A' }}</p>
            <p class="op">Sex: {{ $patientprint->gender ?? 'N/A' }}</p>
            </div>
        </div>
        <div class="col-md-4" style="margin-left: 25px">
            <div class="row">
                <p class="op">Doctor's Name: {{ $patientprint->consultent ?? 'N/A' }}</p>
                <p class="op">Room No: {{ $patientprint->roomno ?? 'N/A' }}</p>
                <p class="op">Receipt type: {{ $patientprint->receipttype ?? 'N/A' }}</p>
                <p class="op">Taka: {{ $patientprint->fees ?? 'N/A' }}</p>
                <p class="op">Serial: {{ $count->slno }}</p>
            </div>
        </div>
        <div class="col-md-2" style="margin-left: 50px">
            <div class="row">
                <p class="op">Bill No: {{ $patientprint->billno ?? 'N/A' }}</p>
                <p class="op">Reg No: {{ $patientprint->regno ?? 'N/A' }}</p>
                <p class="op">Date: {{ now()->format('Y-m-d') }}</p>
                <p class="op">Time: {{ now()->setTimezone('Asia/Dhaka')->format('h:i:s A') }}</p>
                <p class="op">User: {{ $username }}</p>
            </div>
        </div>
      </div>
   </main>
